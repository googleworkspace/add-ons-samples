...
def createClient() -> Flow:
    """Creates a new OAuth2 client with the configured keys."""
    oauth2Client = Flow.from_client_secrets_file(
        CLIENT_SECRETS_FILE, scopes=SCOPES)
    oauth2Client.redirect_uri = KEYS["redirect_uris"][0]
    return oauth2Client
...
def generate_auth_url(user_name: str, config_complete_redirect_url: str) -> str:
    ...
    auth_url, _ = oauth2Client.authorization_url(
...
def oauth2callback(url: str):
    ...
    oauth2Client = createClient()
    oauth2Client.fetch_token(code=qs["code"][0])
