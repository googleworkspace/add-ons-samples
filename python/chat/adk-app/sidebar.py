...
# TODO: Chat API is only used for dev purposes (mock sidebar card with a single Chat message)
...
def create_mock_message() -> str:
    return google_chat_cloud_client.create_message(google_chat.CreateMessageRequest(
        parent = spaceName,
        message = { "text": "Message used as a mock of GWAO sidebar..." }
    )).name

messageName = create_mock_message()
turn_card_sections = []
function_call_card_section_map = {}

def update_mock_message():
    return google_chat_cloud_client.update_message(google_chat.UpdateMessageRequest(
        message = { "name": messageName, "cards_v2": [{ "card": { "sections": turn_card_sections[::-1] }}]},
        update_mask = "cardsV2"
    ))

def create_or_update_section(author="Agent", function_call_id=None, text="", widgets=[], final=True):
    ...
    if function_call_id != None:
        print(f"Handing function call with ID: {function_call_id}...")
        if function_call_card_section_map.get(function_call_id) is None:
            print(f"Function call ID {function_call_id} is new, creating new section #{len(turn_card_sections)}...")
            function_call_card_section_map[function_call_id] = len(turn_card_sections)
            turn_card_sections.append({ "widgets": []})
        print(f"Updating the turn card section #{function_call_card_section_map.get(function_call_id)}...")
        turn_card_sections[function_call_card_section_map.get(function_call_id)] = { "widgets": textWidgets + widgets + (create_status_accessory_widgets() if final is False else []) }
    elif final is True:
        print(f"Handing final answer...")
        turn_card_sections.append({ "widgets": textWidgets })
...
def create_destination_widgets(destinations=[]) -> list:
    carousel_cards = []
...
def create_place_widgets(places=[]) -> list:
    carousel_cards = []
    for item in places:
        carousel_card_widgets = []
        footer_widgets = []
...
def create_source_widgets(text="") -> list:
...
async def request_adk_agent(message, clean=True):
    ...
    for event_raw in ai_agent.stream_query(user_id=userName, session_id=session_id, message=message):
        ...
        if "text" in event["content"]["parts"][0]:
            ...
            print(f"\n{author} {text}")
            # TODO: Update new Chat message with final text, thoughts, and done status
            create_or_update_section(author=author, text=text, widgets=[])
                ...
                match name:
                    ...
                    case "place_agent":
                        ...
                        create_or_update_section(
                            ...
                            text="",
                            widgets=[]
                            # TODO: widgets=create_destination_widgets(response["places"])
                        )
                    case "map_tool":
                        ...
                        create_or_update_section(
                            ...
                            text="",
                            widgets=[]
                            # TODO: widgets=create_place_widgets(response["places"])
                        )
                    case "google_search_grounding":
                        ...
                        create_or_update_section(
                            ...
                            function_call_id=id,
                            text="",
                            widgets=create_source_widgets(response["result"])
                        )
...
if __name__ == "__main__":
    ...
    asyncio.run(request_adk_agent(get_content_from_chat_message_payload(scenario_1_turn_1), True))
